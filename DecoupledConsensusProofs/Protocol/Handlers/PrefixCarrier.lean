...
namespace DecoupledConsensusModel.Proofs.NamedOutageHistory.PrefixCarrier
...
/-- Event-index instance of the copied, checked prefix invariant. -/
theorem justification_carrier_stateBefore
    (S : Setup V) (rho : NamedRun V) (i : Nat) (v : V) :
    ∃ C : NamedBlock V, C ∈ (NamedRun.stateBefore S rho i v).st.bodies ∧
      (derive_named S.E S.cfg C).J = (NamedRun.stateBefore S rho i v).st.core.J ∧
      (derive_named S.E S.cfg C).h_j = (NamedRun.stateBefore S rho i v).st.core.h_j :=
...
#print axioms justification_carrier_stateBefore
end DecoupledConsensusModel.Proofs.NamedOutageHistory.PrefixCarrier
