...
/-! Compiler-free healthy-prefix reverse late raw vote-or-equivocation relay.
The copied SG metadata/clock helpers are private. Only the final query is public. -/
namespace DecoupledConsensusModel.Proofs.NamedHealthyLateRelay
open Execution Internal.NamedOutageEntry DecoupledConsensusModel.Protocol
...
private theorem late_g1_delay_eq_g2 (S : Setup V) (r : Round) :
    late S.E S.hc r .g1 + S.E.Δ = late S.E S.hc r .g2 := by
  unfold late Phase.lateOffset
  ring


private theorem late_g2_lt_opening (S : Setup V) (r : Round) :
    late S.E S.hc r .g2 < opening S.E S.hc r := by
  unfold late Phase.lateOffset
  change opening S.E S.hc r + (-1 : Time) * S.E.Δ < opening S.E S.hc r
  have hneg : (-1 : Time) * S.E.Δ < 0 :=
    Int.mul_neg_of_neg_of_pos (by norm_num) S.E.Δ_pos
  calc
    opening S.E S.hc r + (-1 : Time) * S.E.Δ < opening S.E S.hc r + 0 :=
...
private theorem held_projection_raw_after_event (S : Setup V) (rho : NamedRun V)
    ...
    {j : Nat} {e : NamedEvent V} (he : rho.events[j]? = some e)
    (ht : e.time < late S.E S.hc r .g2) (b : NamedAttestation V)
    ...
    Protocol.sgVote b.erase ∈ DecoupledConsensusModel.Protocol.rawInputs n.st.core.toHealing.gradeView
      S.hc.η_SG r (late S.E S.hc r .g2) sender := by
  let cut := domain S.E S.hc r .g2
  ...
  have hcut : e.time < cut := by
    change e.time < opening S.E S.hc r + (-1 : Int) * S.E.Δ
    simpa only [late, Phase.lateOffset] using ht
...
theorem accepted_sg_vote_or_equiv_at_g2_late_of_relay
    ...
    ∀ reader ∈ rho.honest,
      ...
      let raw := DecoupledConsensusModel.Protocol.rawInputs n.st.core.toHealing.gradeView
        S.hc.η_SG r (late S.E S.hc r .g2) a.val_index
  ...
  have htaLate : ta < late S.E S.hc r .g2 :=
    hsourceCut.trans (by
      rw [← late_g1_delay_eq_g2 S r]
  ...
  · have hmissing : NamedReceipt.processed
    ...
    have htdOpen : td < opening S.E S.hc r :=
      htdLate.trans (late_g2_lt_opening S r)
    ...
    have heLate : e.time < late S.E S.hc r .g2 := by
...
#print axioms accepted_sg_vote_or_equiv_at_g2_late_of_relay
...
end DecoupledConsensusModel.Proofs.NamedHealthyLateRelay
