...
namespace DecoupledConsensusModel.Proofs.NamedSGArrival
...
private theorem carry_refl (st : Protocol.NamedStore V) : Carry st st :=
  fun _ ha => ⟨ha, rfl⟩

omit [DecidableEq V] [Fintype V] in
private theorem carry_trans {st mid out : Protocol.NamedStore V}
...
omit [DecidableEq V] [Fintype V] in
private theorem carry_fields {st out : Protocol.NamedStore V}
...
private theorem rows_carry (S : Setup V) (st : Protocol.NamedStore V)
...
private theorem block_carry (S : Setup V) (st : Protocol.NamedStore V) (B : NamedBlock V)
    (hcoh : Proofs.NamedStore.Coherent S.E S.cfg st) :
...
private theorem propose_carry (gc : Protocol.GradeContract V) (S : Setup V)
    (nd : Protocol.Node V) (st : Protocol.NamedStore V) (hcoh : Proofs.NamedStore.Coherent S.E S.cfg st) :
...
private theorem gf_carry (gc : Protocol.GradeContract V) (S : Setup V)
...
private theorem confirmation_carry (gc : Protocol.GradeContract V) (S : Setup V)
...
private theorem tick_carry (gc : Protocol.GradeContract V) (S : Setup V) (nd : Protocol.Node V)
...
private theorem process_carry (S : Setup V) (st : Protocol.NamedStore V) (o : NamedObject V)
...
private theorem carry_succ (S : Setup V) (rho : NamedRun V) (i : Nat) (reader : V) :
...
/-- **Public twin of the prefix `Data` fold**. A full named
row held at the read just after event `i` is in its own bucket and already
carries a projected receipt stamp no later than that event's time. Only this
reading of the private fold is exported. -/
theorem held_row_own_and_stamp_at_event (S : Setup V) (rho : NamedRun V)
    (sch : NamedScheduleWellFormed S rho) {i : Nat} {e : NamedEvent V}
    (he : rho.events[i]? = some e) (reader : V) (k : Round) (a : NamedAttestation V)
    (ha : a ∈ (NamedRun.stateBefore S rho (i + 1) reader).st.sg_rows k) :
    a.round = k ∧ ∃ stamp : Time, stamp ≤ e.time ∧
      (NamedRun.stateBefore S rho (i + 1) reader).st.core.timestamp_sg_vote
        (Protocol.sgVote a.erase) = some (stamp : Stamp) :=
  (data_at_event S rho sch he reader).2.2 k a ha
...
private theorem delivery_round (S : Setup V) (rho : NamedRun V)
...
private theorem actual_call_input (S : Setup V) (rho : NamedRun V)
...
private theorem honest_input_pool_row (S : Setup V) (rho : NamedRun V)
...
/-- **A delivered honest row is retained with its own payload** (
generalised). Exposed for the delivery-to-pool wrapper in
`NamedEmissionShape`: honesty plus the delivery window rule out every branch of
the handler's guard,
so no side condition reaches the caller. Statement unchanged. -/
...
private theorem source_deadline_le_next_early (S : Setup V) (q : Round) (hR : 3 ≤ S.hc.R) :
    S.a q + S.E.Δ ≤ DecoupledConsensusModel.Protocol.early S.E S.hc (q + 1) .g2 := by
...
private theorem early_le_domain (S : Setup V) (q : Round) :
...
private theorem previous_round_in_window (S : Setup V) (q : Round) :
...
theorem healthy_emitted_sg_at_next_g2_of_deadline (S : Setup V) (rho : NamedRun V)
    ...
    ∀ reader ∈ rho.honest,
      ...
      Protocol.sgVote a.erase ∈ DecoupledConsensusModel.Protocol.rawInputs n.st.core.toHealing.gradeView
        S.hc.η_SG r (DecoupledConsensusModel.Protocol.early S.E S.hc r .g2) a.val_index := by
  intro reader hreader
  let r := a.round + 1
  let cut := DecoupledConsensusModel.Protocol.domain S.E S.hc r .g2
  let early := DecoupledConsensusModel.Protocol.early S.E S.hc r .g2
  let n := NamedRun.stateBeforeTime S rho cut reader
  have hdeadline := hdead
  obtain ⟨td, hlo, hhi, i, hcall⟩ :=
    hexec.healthy.broadcast a.val_index ha (.attest a) (S.a a.round) hem
      reader hreader hdeadline rfl
  have hpost := honest_row_after_call S rho hexec.core.toNamedScheduleWellFormed
    hexec.core.toNamedUnforgeable ha hem hcall hlo hhi
  obtain ⟨e, he, _, het⟩ := hcall.2
  have hdata : Data td (NamedRun.stateBefore S rho (i + 1) reader).st := by
    simpa only [het] using (data_at_event S rho hexec.core.toNamedScheduleWellFormed he reader).2
  obtain ⟨_, stamp, hstampBound, hstamp⟩ := hdata.2 a.round a hpost
  have htdEarly : td < early := hhi.trans_le
    (source_deadline_le_next_early S a.round S.hc.R_ge_three)
  have htdCut : td < cut := htdEarly.trans_le (early_le_domain S r)
  have heCut : e.time < cut := by simpa only [het] using htdCut
  have hindex := post_index_le_strict_filter_length rho hexec.core.sorted cut he heCut
  obtain ⟨hretained, hstampRetained⟩ := stateBefore_sg_row_stamp_mono S rho reader hindex hpost
  have hread : NamedRun.stateBeforeTime S rho cut =
      NamedRun.stateBefore S rho (rho.events.filter (fun e => decide (e.time < cut))).length := by
    exact congrArg (fun events => events.foldl (NamedWorld.step S) NamedWorld.init)
      (strict_filter_eq_take rho hexec.core.sorted cut)
  have hheld : a ∈ n.st.sg_rows a.round := by simpa only [n, hread] using hretained
  have hreceipt : n.st.core.timestamp_sg_vote (Protocol.sgVote a.erase) = some (stamp : Stamp) := by
    simpa only [n, hread] using hstampRetained.trans hstamp
  have hstampEarly : stamp < early := hstampBound.trans_lt htdEarly
  have hoccur : occurrenceBefore
      (n.st.core.timestamp_sg_vote (Protocol.sgVote a.erase)) early = true := by
  ...
  have hrawPool : a.erase ∈ n.st.core.sg_pool a.round :=
    NamedAdmission.pool_view_mem n.st hcoh.2.2.2.1 a hheld
  have hsg : Protocol.sgVote a.erase ∈ n.st.core.toHealing.gradeView.sg_votes a.round :=
    Finset.mem_image_of_mem Protocol.sgVote hrawPool
  refine ⟨hheld, hoccur, ?_⟩
  apply Finset.mem_filter.mpr
  refine ⟨Finset.mem_biUnion.mpr ?_, rfl, hoccur⟩
  exact ⟨a.round, List.mem_toFinset.mpr (previous_round_in_window S a.round), hsg⟩


end DecoupledConsensusModel.Proofs.NamedSGArrival
