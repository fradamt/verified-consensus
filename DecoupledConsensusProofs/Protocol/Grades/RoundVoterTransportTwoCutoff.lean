...
/-!
# GST-zero round-voter transport through the two-cutoff interface

`TwoCutoffDelivery` starts at the early G2 cutoff and therefore does not carry
an action emission to that cutoff. At GST zero, core synchrony supplies the
earlier action-to-G2 hop directly. The two theorems below expose the interface
requested by T3 while retaining the K6 action-read case used for the carrier
floor.
-/
...
private theorem carrier_time_le_of_key_le {e f : NamedEvent V}
...
omit [DecidableEq V] [Fintype V] in
private theorem carrier_strict_filter_eq_take
    (rho : NamedRun V)
  ...
  have hdown : ∀ e f : NamedEvent V, e.key ≤ f.key →
    ...
    exact (carrier_time_le_of_key_le hkey).trans_lt hf
...
omit [DecidableEq V] [Fintype V] in
private theorem carrier_post_index_le_strict_filter_length
    (rho : NamedRun V)
    (hsorted : rho.events.Pairwise (fun e f => e.key ≤ f.key))
  ...
  have htake :
    ...
    · obtain ⟨hkLen, hkGet⟩ := List.getElem?_eq_some_iff.mp hkRun
      ...
      exact (carrier_time_le_of_key_le hkey).trans_lt ht
...
private theorem carrier_post_event_body_at_strict_read
  ...
  have hindex := carrier_post_index_le_strict_filter_length
  ...
  rw [carrier_strict_filter_eq_take rho core.sorted cut]
  exact hcarry

private theorem carrier_strict_read_eq_index
    (S : Setup V) (rho : NamedRun V) (core : NamedAdmissibleCore S rho)
    (t : Time) :
  ...
  congrArg (fun events => events.foldl (NamedWorld.step S) NamedWorld.init)
    (carrier_strict_filter_eq_take rho core.sorted t)

omit [DecidableEq V] [Fintype V] in
private theorem carrier_strict_lengths_mono
...
private theorem carrier_strict_body_stamp_carry
    (S : Setup V) (rho : NamedRun V) (core : NamedAdmissibleCore S rho)
    ...
    H ∈ (NamedRun.stateBeforeTime S rho late reader).st.bodies ∧
      (NamedRun.stateBeforeTime S rho late reader).st.core.timestamp_block
        ...
        (NamedRun.stateBeforeTime S rho early reader).st.core.timestamp_block
          H.erase := by
  rw [carrier_strict_read_eq_index S rho core early] at hH
  rw [carrier_strict_read_eq_index S rho core late,
    carrier_strict_read_eq_index S rho core early]
  exact NamedBlockStamp.stateBefore_body_stamp_mono S rho reader
    (carrier_strict_lengths_mono rho ht) hH

private theorem carrier_runBlock_unique_of_erase_eq
...
private theorem carrier_held_find_at_strict
    (S : Setup V) (rho : NamedRun V) (core : NamedAdmissibleCore S rho)
...
/-- A synchronized carrier body is ready under compatibility with the later
finalized root. If finality already passed the carrier, parent closure shows
that the receiver held the body before the delivery call. -/
private theorem compatible_carrier_body_at_read_after_gst
    (S : Setup V) (rho : NamedRun V) (core : NamedAdmissibleCore S rho)
  ...
  have hEarly :
    ...
    · obtain ⟨n, hn, hbefore⟩ := Proofs.NamedRuntime.stateBeforeTime_eq_prefix
      ...
      · obtain ⟨eSource, heSource, -, htSource⟩ := hacc.1.2
        ...
        · exact carrier_post_event_body_at_strict_read
        ...
        · have hmissing : NamedReceipt.processed
          ...
          · have hFcall : Block.Preceq
              ...
              have hfuture : Block.Preceq
                ...
                simpa only [carrier_strict_read_eq_index
              ...
              · rcases Block.preceq_linear hfuture hHF with h | h
                ...
                · have hFmem := Proofs.NamedStoreBridge.finalizedInTree_stateBefore
                  ...
                  have hKH : K = H := carrier_runBlock_unique_of_erase_eq
  ...
  obtain ⟨hHeld, hCarry⟩ := carrier_strict_body_stamp_carry
  ...
  · simpa only [Proofs.NamedWire.erase_root] using
      (carrier_held_find_at_strict S rho core hreader targetRead H hHeld)



/-- An honest action vote from any eligible expiry-window round is an exact
interpreted carrier at a later phase read. Compatibility with the reader's
finalized root is the only body-open input. -/
theorem honestRoundVote_interpreted_at_reader_of_twoCutoff_compatible
    (S : Setup V) (rho : NamedRun V) (core : NamedAdmissibleCore S rho)
    (hGST : S.E.t_GST = 0)
    (_delivery : TwoCutoffDelivery S rho q)
    (q k : Round) (p : Phase) (hk : k ∈ Protocol.latest_window S.hc.η_SG q)
    (w : V) (hw : w ∈ rho.honest)
  ...
  have hcompatK : Block.compatible K.erase target.st.core.F = true := by
    ...
    simpa only [target, Block.compatible, Bool.or_eq_true, or_comm]
      using hcompat
  have hgst : S.E.t_GST ≤ S.a k := by
    rw [hGST]
    exact Proofs.HealingLemmas.a_nonneg S k
  have hmaxDeadline : max (S.a k) S.E.t_GST + S.E.Δ ≤
      early S.E S.hc q p := by
    rw [max_eq_left hgst]
    exact hdeadline
  obtain ⟨_, hstamp, hfind⟩ :=
    compatible_carrier_body_at_read_after_gst S rho core
...
#print axioms honestRoundVote_interpreted_at_reader_of_twoCutoff_compatible
